...
<?php
	...
	if(isWeekLocked($week['Week']['lock_time']) == true) {
		...
		$fDisabled = isPositionLocked($locks, $this->request->data, 'F');
	...
	echo "<tr>";
	echo "<td>RB/WR/TE</td>";
	echo "<td>".$this->Form->input("f_id", array('label' => '', 'type' => 'select', 'options' => $players['F'], 'empty' => '', 'disabled' => $fDisabled))."</td>";
	printColumns('f', $playerentries);
	echo "</tr>";
