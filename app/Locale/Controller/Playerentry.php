<?php
	class PlayerEntry extends AppModel {
		...
		public $virtualFields = array('game_id');
		...
		public function espnParser($weekId) {
			...
			//$playerEntries = $this->getPlayerEntriesByWeek($weekId);
			...
			array_push($gameIdsArray, '400756919');
			
			for($i = 0; $i < count($gameIdsArray); $i++) {
				$playerEntries = array();
				$playerEntries = $this->espnProcessBoxScore($gameIdsArray[$i], $playerEntries, $weekId);
		...
		private function espnProcessBoxScore($gameId, $playerEntries, $weekId) {
			echo $gameId."\n";
			$schools = array_unique($this->Player->find('list', array('fields' => array('school'))));
			$url = "http://espn.go.com/college-football/boxscore?gameId=".$gameId;
			$html = file_get_html($url);
